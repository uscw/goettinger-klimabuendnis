...
tdelta = 1 # timedelta to next event to post 
    ...
    def content_separation(content_lines):
        ...
        for line in content_lines:
            if line == "---\n":
                fm_delims.append(k)
            k += 1
        frontmatter = content_lines[fm_delims[0]:fm_delims[1]+1]
        content = content_lines[fm_delims[1]+1:]
...
def main():
    ...
    for file in os.listdir(eventFDIR):
        if file.startswith(dt_now):
            ...
            print (content)
            sm_pub = False
            sm_fm_entry = "no frontmatter entry"
            ...
            if sm_pub == False:
                print ("no publication wanted by frontmatter: " + sm_fm_entry)
                sys.exit(0)
