...
var arQuestionsOptFullPro = new Array();  // erste Option zur Beantwortung der Frage
var arQuestionsOptPro = new Array();      // zweite Option zur Beantwortung der Frage
var arQuestionsOptNeutral = new Array();  // dritte Option zur Beantwortung der Frage
var arQuestionsOptContra = new Array();  // vierte Option zur Beantwortung der Frage
var arQuestionsOptFullContra  = new Array();  // fuenfte Option zur Beantwortung der Frage
var arPartyPositions = new Array();	// Position der Partei als Zahl aus den CSV-Dateien (2/1/0/-1/-2)
var arPartyOpinions = new Array();		// Begründung der Parteien aus den CSV-Dateien
var arPersonalPositions = new Array();	// eigene Position als Zahl (2/1/0/-1/-2)
var arVotingDouble = new Array();	// eigene Position als Zahl (4/2/0/-2/-4)
...
//	var numberOfPositions = arPartyPositions.length; // 12 = 3 Fragen * 4 Parteien
	...
	var indexPartyInArray = -1; // Berechnung der Position des Index der aktuellen Partei
        ...
        var matchQuestion;
        var maxMatch = 4;       // 2 bei drei, 4 bei fünf Möglichkeiten
...
//	for (i = 0; i <= (arPartyFiles.length-1); i++)
	...
	for (i = 0; i <= (numberOfPositions-1); i++)
	{
		...
		if ( (arPersonalPositions[modulo] < 99) ) 
		{
		    ...
		    // je nach Abstand der persönlichen Meinung von der Partei-Antwort den Zaehler von 0 (völlig anders) bis 1 (gleiche Antwort) erhoehen.
                    //  options=5, maxMatch=4      or  options=3, maxMatch=2
		    // a\b || 2 | 1 | 0 | -1| -2|
		    // --------------------------       a\b || 1 | 0 | -1|
		    //   2 || 4 | 3 | 2 | 1 | 0 |       ------------------
		    //   1 || 3 | 4 | 3 | 2 | 1 |  or     1 || 2 | 1 | 0 |
		    //   0 || 2 | 3 | 4 | 3 | 2 |         0 || 1 | 2 | 1 |
		    //  -1 || 1 | 2 | 3 | 4 | 3 |        -1 || 0 | 1 | 2 |
		    //  -2 || 0 | 1 | 2 | 3 | 4 |
		    // die Werte in den Spalten errechnen sich aus: maxMatch - |a-b|
                    matchQuestion = maxMatch - Math.abs(arPartyPositions[i] - arPersonalPositions[modulo]);
		    positionsMatch+=matchQuestion*faktor/maxMatch;
	...
	for(i = 0; i <= arZeilen.length-1; i++)
	{
		...
		if (modus == 1)
		{
			...
			arQuestionsLong.push(valueTwo);
		        if ( arZeilen[i].length == 7 )
		        {
			    arQuestionsOptFullPro.push(arZeilen[i][2]);
			    arQuestionsOptPro.push(arZeilen[i][3]);
			    arQuestionsOptNeutral.push(arZeilen[i][4]);
			    arQuestionsOptContra.push(arZeilen[i][5]);
			    arQuestionsOptFullContra.push(arZeilen[i][6]);
			} else {
			    arQuestionsOptFullPro.push("5");
			    arQuestionsOptPro.push(arZeilen[i].length);
			    arQuestionsOptNeutral.push(arZeilen[i].length);
			    arQuestionsOptContra.push(arZeilen[i].length);
			    arQuestionsOptFullContra.push("1");
			}
		}
...
//				console.log("i: "+i+ " value: "+valueTwo)
			...
			else if (valueTwo)
			{
				...
				arPartyPositions.push(valueOne); // -2,-1,0,1,2
...
// v.0.3 NEU
// ersetzt die Position (-2 , -1 , 0, 1, 2) mit dem passenden Button
function fnTransformPositionToButton(position)
{
	// var arButtons = new Array("btn-danger","btn-secondary","btn-warning","btn-primary","btn-success")
	var arButtons = new Array("btn-blue1","btn-blue2","btn-blue3","btn-blue4","btn-blue5")
	var positionButton = "btn-default";
	for (z = -2; z <= 2; z++)
	{
	 	if (z == position)
		{
			positionButton = arButtons[(z+2)];
		}
	}
...
// v.0.3 NEU
// ersetzt die Position (-2 , -1 , 0, 1, 2) mit dem passenden Icon
function fnTransformPositionToIcon(position)
{
	// var arIcons = new Array("&#x2716;","&#x21D8;","&#x25EF;","&#x21D7;","&#x2714;")
	// var arIcons = new Array("&#x2B24;","&#x25D5;","&#x25D2;","&#x25D4;","&#x25EF;")
        // var arIcons = new Array("&#x2460;","&#x2461;","&#x2462;","&#x2463;","&#x2464;")
        var arIcons = new Array("&#x45;","&#x44;","&#x43;","&#x42;","&#x41;")
	var positionIcon = "&#x21B7;";
	for (z = -2; z <= 2; z++)
	{
	 	if (z == position)
		{
			positionIcon = arIcons[(z+2)];
		}
	}
	return positionIcon;
}

// ersetzt die Partei-Position (-2 , -1 , 0, 1, 2) mit der passenden Farbe
function fnTransformPositionToColor(position)
{
	...
	// var arColors = new Array("#d9534f","#909090","#f0ad4e","#0053ff","#5cb85c")
	var arColors = new Array("#d3d7df","#a7c2d8","#85b1ee","#007bff","#0541bb")
	var positionColor = "#c0c0c0";
	for (z = -2; z <= 2; z++)
	{
	 	if (z == position)
		{
			positionColor = arColors[(z+2)];
		}
	}
...
// ersetzt die Partei-Position (-2 , -1 , 0, 1, 2) mit dem passenden Text
function fnTransformPositionToText(position)
{
	var arText = new Array("[e]","[d]","[c]","[b]","[a]")
	var positionText = "[/]";
	for (z = -2; z <= 2; z++)
	{
	 	if (z == position)
		{
			positionText = arText[(z+2)];
		}
	}
	return positionText;
...
//	var positionImage = fnTransformPositionToImage(arPersonalPositions[i]);
	...
	$("#selfPosition"+i).removeClass("btn-danger btn-secondary btn-warning btn-success btn-primary btn-default").addClass(positionButton);
